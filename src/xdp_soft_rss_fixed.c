// clang -O2 -target bpf -g -c xdp_soft_rss_fixed.c -o xdp_soft_rss_fixed.o
#include <linux/bpf.h>
...
int xdp_soft_rss(struct xdp_md *ctx) {
    ...
    // Check if this is a loopback packet (no Ethernet header)
    // Loopback packets on Linux start with a 4-byte packet type field
    if (data_end - data >= 4) {
        __u32 *loopback_hdr = data;
        __u32 packet_type = *loopback_hdr;

        // Loopback packets have packet_type in host byte order
        // 0x00000001 = IPv4, 0x00000002 = IPv6
        if (packet_type == 0x01000000) {  // IPv4 in little endian
            ...
            ip = data + offset;
            proto = ip->protocol;
        } else if (packet_type == 0x02000000) {  // IPv6 in little endian
            // IPv6 loopback packet - skip for now
            return XDP_PASS;
        } else {
            // Try Ethernet parsing
            goto try_ethernet;
        }
    } else {
        goto try_ethernet;
...
process_ip:
    ...
    // For loopback packets, offset = 4; for Ethernet, offset includes Ethernet header
    if (!ip)
        return XDP_PASS;
    ...
    if (offset == 4) {
        ...
        // Ethernet packet: transport starts after offset we already calculated
